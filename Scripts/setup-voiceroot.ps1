...
function DeployLatestArtifact() {
    ...
    # Extract the parent folder and the leaf folder from $TargetDir
    $parentDir = Split-Path $TargetDir -Parent
    $leafDir = Split-Path $TargetDir -Leaf
    $parentLeaf = Split-Path $parentDir -Leaf

    # If the leaf folder matches the parent's folder name, remove the duplicate segment.
    if ($leafDir -eq $parentLeaf) {
        Write-Host "Duplicate folder segment '$leafDir' detected. Removing duplicate from TargetDir..."
        $script:targetDir = $parentDir
    }
