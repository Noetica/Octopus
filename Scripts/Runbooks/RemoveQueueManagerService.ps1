...
function CommentInfLine {
    ...
    try {
        ...
        $modifiedLines | Set-Content -Path $FilePath -Encoding Default -ErrorAction Stop
...
$isServiceCommented = IsServiceCommented `
    ...
    -SearchText "Queue Manager.exe"

if($isServiceCommented) {
    Write-Output "Queue Manager Service is not active. Exiting script."
    exit
}

ControlService -apiRequested "Queue Manager Service" -operation "Stop"
